<template>
  <div>
    <div class="container">
      <aside class="style" v-if="currentPath == true">
        <ul v-on:click="name()">
          <li>
            <img src="./../../../public/img/icons/android-chrome-192x192.png" />
            <p>پرهام الاهیار</p>
            <p>۰۹۹۱۲۸۹۹۴۷۹</p>
          </li>

          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="1.25rem"
              height="1.25rem"
              fill="currentColor"
              class="bi bi-chat"
              viewBox="0 0 16 16"
            >
              <path
                d="M2.678 11.894a1 1 0 0 1 .287.801 10.97 10.97 0 0 1-.398 2c1.395-.323 2.247-.697 2.634-.893a1 1 0 0 1 .71-.074A8.06 8.06 0 0 0 8 14c3.996 0 7-2.807 7-6 0-3.192-3.004-6-7-6S1 4.808 1 8c0 1.468.617 2.83 1.678 3.894zm-.493 3.905a21.682 21.682 0 0 1-.713.129c-.2.032-.352-.176-.273-.362a9.68 9.68 0 0 0 .244-.637l.003-.01c.248-.72.45-1.548.524-2.319C.743 11.37 0 9.76 0 8c0-3.866 3.582-7 8-7s8 3.134 8 7-3.582 7-8 7a9.06 9.06 0 0 1-2.347-.306c-.52.263-1.639.742-3.468 1.105z"
              />
            </svg>
            <router-link to="/user/profile/comment"> نظرات </router-link>
          </li>
          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="1.25rem"
              height="1.25rem"
              fill="currentColor"
              class="bi bi-signpost-2"
              viewBox="0 0 16 16"
            >
              <path
                d="M7 1.414V2H2a1 1 0 0 0-1 1v2a1 1 0 0 0 1 1h5v1H2.5a1 1 0 0 0-.8.4L.725 8.7a.5.5 0 0 0 0 .6l.975 1.3a1 1 0 0 0 .8.4H7v5h2v-5h5a1 1 0 0 0 1-1V8a1 1 0 0 0-1-1H9V6h4.5a1 1 0 0 0 .8-.4l.975-1.3a.5.5 0 0 0 0-.6L14.3 2.4a1 1 0 0 0-.8-.4H9v-.586a1 1 0 0 0-2 0zM13.5 3l.75 1-.75 1H2V3h11.5zm.5 5v2H2.5l-.75-1 .75-1H14z"
              />
            </svg>

            <router-link to="/user/profile/address"> آدرس ها </router-link>
          </li>
          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="1.25rem"
              height="1.25rem"
              fill="currentColor"
              class="bi bi-suit-heart"
              viewBox="0 0 16 16"
            >
              <path
                d="m8 6.236-.894-1.789c-.222-.443-.607-1.08-1.152-1.595C5.418 2.345 4.776 2 4 2 2.324 2 1 3.326 1 4.92c0 1.211.554 2.066 1.868 3.37.337.334.721.695 1.146 1.093C5.122 10.423 6.5 11.717 8 13.447c1.5-1.73 2.878-3.024 3.986-4.064.425-.398.81-.76 1.146-1.093C14.446 6.986 15 6.131 15 4.92 15 3.326 13.676 2 12 2c-.777 0-1.418.345-1.954.852-.545.515-.93 1.152-1.152 1.595L8 6.236zm.392 8.292a.513.513 0 0 1-.784 0c-1.601-1.902-3.05-3.262-4.243-4.381C1.3 8.208 0 6.989 0 4.92 0 2.755 1.79 1 4 1c1.6 0 2.719 1.05 3.404 2.008.26.365.458.716.596.992a7.55 7.55 0 0 1 .596-.992C9.281 2.049 10.4 1 12 1c2.21 0 4 1.755 4 3.92 0 2.069-1.3 3.288-3.365 5.227-1.193 1.12-2.642 2.48-4.243 4.38z"
              />
            </svg>
            <router-link to="/foo"> لیست ها </router-link>
          </li>

          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="1.25rem"
              height="1.25rem"
              fill="currentColor"
              class="bi bi-basket3"
              viewBox="0 0 16 16"
            >
              <path
                d="M5.757 1.071a.5.5 0 0 1 .172.686L3.383 6h9.234L10.07 1.757a.5.5 0 1 1 .858-.514L13.783 6H15.5a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5H.5a.5.5 0 0 1-.5-.5v-1A.5.5 0 0 1 .5 6h1.717L5.07 1.243a.5.5 0 0 1 .686-.172zM3.394 15l-1.48-6h-.97l1.525 6.426a.75.75 0 0 0 .729.574h9.606a.75.75 0 0 0 .73-.574L15.056 9h-.972l-1.479 6h-9.21z"
              />
            </svg>
            <router-link to="/user/profile/order"> سفارش های من </router-link>
          </li>
          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="1.25rem"
              height="1.25rem"
              fill="currentColor"
              class="bi bi-person"
              viewBox="0 0 16 16"
            >
              <path
                d="M8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6zm2-3a2 2 0 1 1-4 0 2 2 0 0 1 4 0zm4 8c0 1-1 1-1 1H3s-1 0-1-1 1-4 6-4 6 3 6 4zm-1-.004c-.001-.246-.154-.986-.832-1.664C11.516 10.68 10.289 10 8 10c-2.29 0-3.516.68-4.168 1.332-.678.678-.83 1.418-.832 1.664h10z"
              />
            </svg>
            <router-link to="/user/profile/info"> اطلاعات حساب </router-link>
          </li>
        </ul>
      </aside>
    </div>

    <router-view v-if="currentPath == false" />
  </div>
</template>





<script>
export default {
  data() {
    return {
      currentPath: true,
    };
  },

  beforeUpdate() {
    if (this.$route.path == "user/profile") {
      this.currentPath = true;
    } else this.currentPath = false;
  },

  methods: {
    name: function () {
      if (this.$route.path == "user/profile") {
        return (this.currentPath = true);
      } else return (this.currentPath = false);
    },
  },
};
</script>





<style>
.container {
  margin-top: 1rem;
  margin-left: 1rem;
  margin-right: 1rem;
}

aside {
  width: 100vw;
  background-color: #fff;
  height: 100vh;
  border-radius: 20px;
}

ul {
  list-style-type: none;
}
ul > li {
  padding: 10px;
  font-size: 1.25rem;
  color: #464b53;
}
ul > li > img {
  border-radius: 50%;
  background-color: violet;
  width: 64px;
  height: 64px;
}
ul > li > svg {
  vertical-align: middle;
  margin-left: 1rem;
  color: #464b53;
}
ul > li > p {
  color: #464b53;
}
article {
  background-color: blue;
  height: 100vh;
  border-radius: 20px;
}
</style>